...
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "web_front_src", "static", "polymarket.db")
...
def save_settings(private_key: str, funder: str, signature_type: int) -> None:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """
        INSERT OR REPLACE INTO settings (key, private_key, funder, signature_type)
        VALUES ('user_settings', ?, ?, ?)
        """,
        (private_key, funder, signature_type),
    )
    conn.commit()
    conn.close()
...
def get_settings() -> Optional[Dict[str, Any]]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM settings WHERE key = 'user_settings'")
    row = cur.fetchone()
    conn.close()
    if row:
        return dict(row)
